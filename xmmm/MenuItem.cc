...
{
    XtAddCallback(menuitem_, XmNactivateCallback, 
		  (XtCallbackProc)MenuItem::activate_callback,
		  XtPointer(this));
}

MenuItem::MenuItem(::Widget parent, const Xmmm::String &name,
		   const Xmmm::String &label)
{
    menuitem_ = XmCreatePushButton(parent, (char *)name.c(), NULL, 0);
    init_signals();
    postinit();
}

MenuItem::MenuItem(Xmmm::Container &parent, const Xmmm::String &name,
		   const Xmmm::String &label)
{
    menuitem_ = XmCreatePushButton(parent.xt_container(), (char *)name.c(), NULL, 0);
    init_signals();
    postinit();
}
...
{
    XtDestroyWidget(menuitem_);
}

::Widget MenuItem::internal(void)
{
    return menuitem_;
}
