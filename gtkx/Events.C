...
RefPtr<Event> GtkX::translate_event(GdkEvent *in)
{
    bool onoff;
    RefPtr<Event> result;
    switch (in->any.type) {
    ...
    {
	EventKey *out = new EventKey;
	result = out;
	onoff = (in->any.type == GDK_KEY_PRESS);
	// std::cerr << (onoff ? "KeyPress\n" : "KeyRelease\n");
	out->type = (onoff ? KEY_PRESS : KEY_RELEASE);
	out->window = XWindow::wrap(in->key.window);
	out->send_event = in->key.send_event;
	out->time = in->key.time;
	out->state = keystate(in->key.state);
	out->keyval = keyval(in->key.keyval);
	out->length = in->key.length; /* FIXME */
	out->string = in->key.string; /* FIXME survives for lifetime of event? */
	out->hardware_keycode = in->key.hardware_keycode;
	out->group = in->key.group;
	out->is_modifier = in->key.is_modifier;
    }
    break;
    case GDK_BUTTON_PRESS:
    case GDK_BUTTON_RELEASE:
    {
	EventButton *out = new EventButton;
	result = out;
	...
	out->y = in->button.y;
	out->axes = NULL; /* FIXME */
	out->state = buttonstate(in->button.state);
	out->button = in->button.button;
    }
    break;
    case GDK_MOTION_NOTIFY:
    {
	EventMotion *out = new EventMotion;
	result = out;
	...
	out->y = in->motion.y;
	out->axes = NULL; /* FIXME */
	out->state = in->motion.state;
	out->is_hint = in->motion.is_hint;
	out->x_root = in->motion.x_root;
	out->y_root = in->motion.y_root;
    }
    break;
    case GDK_ENTER_NOTIFY:
    case GDK_LEAVE_NOTIFY:
    {
	EventCrossing *out = new EventCrossing;
	result = out;
	...
	out->x = in->crossing.x;
	out->y = in->crossing.y;
	out->x_root = in->crossing.x_root;
	out->y_root = in->crossing.y_root;
	switch(in->crossing.mode) {
	case GDK_CROSSING_NORMAL: out->mode = CROSSING_NORMAL; break;
	case GDK_CROSSING_GRAB: out->mode = CROSSING_GRAB;break;
	case GDK_CROSSING_UNGRAB: out->mode = CROSSING_UNGRAB;break;
	default:
	    std::cerr << "Unknown crossing mode " << in->crossing.mode << "\n";
	    out->mode = CROSSING_NORMAL;
	    break;
	}
	switch(in->crossing.detail) {
	case GDK_NOTIFY_ANCESTOR: out->detail = NOTIFY_ANCESTOR; break;
	case GDK_NOTIFY_VIRTUAL: out->detail = NOTIFY_VIRTUAL; break;
	case GDK_NOTIFY_INFERIOR: out->detail = NOTIFY_INFERIOR; break;
	case GDK_NOTIFY_NONLINEAR: out->detail = NOTIFY_NONLINEAR; break;
	case GDK_NOTIFY_NONLINEAR_VIRTUAL: out->detail = NOTIFY_NONLINEAR_VIRTUAL; break;
	default:
	    std::cerr << "Unknown crossing detail " << in->crossing.detail << "\n";
	    out->detail = NOTIFY_UNKNOWN; 
	    break;
	}
	out->focus = in->crossing.focus;
	out->state = in->crossing.state;
    }
    break;
#if 0
    case FocusIn:
    ...
    case GDK_EXPOSE:
    {
	EventExpose *out = new EventExpose;
	result = out;
	// std::cerr << "Expose\n";
	out->type = EXPOSE;
	...
	out->width = in->expose.area.width;
	out->height = in->expose.area.height;
	out->count = in->expose.count;
    }
    break;
#if 0
    case GraphicsExpose:
    ...
    case GDK_NO_EXPOSE:
    {
	EventNoExpose *out = new EventNoExpose;
	result = out;
	// std::cerr << "NoExpose\n";
	out->type = NO_EXPOSE;
    ...
    {
	EventVisibility *out= new EventVisibility;
	result = out;
    ...
    case GDK_CONFIGURE:
    {
	EventConfigure *out = new EventConfigure;
	result = out;
    ...
    {
	EventSelection *out = new EventSelection;
	result = out;
