...
struct AppData {
    ...
    bool trace;
    const char *play_log;
    const char *bash_init_commands;
    const char *bash_settings;
    const char *dbg_init_commands;
    const char *dbg_settings;
    const char *dbx_init_commands;
    const char *dbx_settings;
    const char *gdb_init_commands;
    ...
    const char *pydb_settings;
    const char *xdb_init_commands;
    const char *xdb_settings;
    const char *restart_commands;
    bool source_init_commands;
    const char *init_symbols;
    const char *vsl_path;
    const char *vsl_library;
    const char *vsl_defs;
    const char *vsl_base_defs;
    const char *default_font;
    const char *variable_width_font;
    const char *fixed_width_font;
    const char *data_font;
    unsigned int default_font_size;
    unsigned int variable_width_font_size;
    unsigned int fixed_width_font_size;
    unsigned int data_font_size;
    const char *font_select_command;
    unsigned int tab_width;
    unsigned int lines_above_cursor;
    unsigned int lines_below_cursor;
    unsigned int indent_source;
    unsigned int indent_script;
    unsigned int indent_code;
    unsigned int line_number_width;
    bool source_editing;
    bool line_buffered_console;
    OnOff     console_has_focus;
    const char *bash_display_shortcuts;
    const char *dbg_display_shortcuts;
    const char *dbx_display_shortcuts;
    const char *gdb_display_shortcuts;
    ...
    const char *data_buttons;
    const char *tool_buttons;
    const char *label_delimiter;
    bool command_toolbar;
    bool common_toolbar;
    bool separate_data_window;
    bool separate_source_window;
    bool separate_exec_window;
    bool data_window;
    bool source_window;
    bool debugger_console;
    bool auto_close_data_window;
    const char *plot_command;
    const char *plot_term_type;
    const char *plot_window_class;
    unsigned int plot_window_delay;
    const char *plot_init_commands;
    const char *plot_2d_settings;
    const char *plot_3d_settings;
    const char *term_command;
    const char *term_type;
    bool use_tty_command;
    int question_timeout;
    int position_timeout;
    int display_timeout;
    bool synchronous_gdb;
    bool terminate_on_eof;
    bool stop_and_continue;
    int run_interrupt_delay;
    int cont_interrupt_delay;
    bool poll_child_status;
    const char *debugger_host;
    const char *debugger_rhost;
    const char *debugger_host_login;
    const char *rhost_init_commands;
    const char *rsh_command;
    bool filter_files;
    const char *list_exec_command;
    const char *list_source_command;
    const char *list_core_command;
    const char *list_dir_command;
    const char *uncompress_command;
    const char *ps_command;
    const char *www_page;
    const char *www_command;
    bool show_invocation;
    bool show_version;
    ...
    const char *get_core_command;
    bool panned_graph_editor;
    bool find_words_only;
    bool find_case_sensitive;
    bool group_iconify;
    bool uniconify_when_ready;
    bool transient_dialogs;
    bool global_tab_completion;
    bool save_history_on_exit;
    bool save_options_on_exit;
    bool cache_source_files;
    bool cache_machine_code;
    bool suppress_warnings;
    bool warn_if_locked;
    unsigned int check_options;
    bool tty_mode;
    OnOff     block_tty_input;
    OnOff     buffer_gdb_output;
    unsigned int annotate;
    bool display_glyphs;
    unsigned int max_glyphs;
    bool cache_glyph_images;
    unsigned int glyph_update_delay;
    bool display_line_numbers;
    bool disassemble;
    unsigned int max_disassemble;
    bool use_source_path;
    bool all_registers;
    bool button_tips;
    bool value_tips;
    bool button_docs;
    ...
    bool startup_tips;
    int startup_tip_count;
    bool status_at_bottom;
    bool toolbars_at_bottom;
    unsigned int max_display_title_length;
    unsigned int max_popup_expr_length;
    unsigned int max_value_tip_length;
    unsigned int max_value_doc_length;
    unsigned int button_tip_delay;
    unsigned int value_tip_delay;
    unsigned int button_doc_delay;
    unsigned int value_doc_delay;
    unsigned int clear_doc_delay;
    unsigned int clear_tip_delay;
    bool blink_while_busy;
    unsigned int busy_blink_rate;
    bool check_grabs;
    unsigned int check_grab_delay;
    unsigned int grab_action_delay;
    const char *grab_action;
    bool detect_aliases;
    bool typed_aliases;
    bool delete_alias_displays;
    bool align_2d_arrays;
    unsigned char array_orientation;
    unsigned char struct_orientation;
    bool show_member_names;
    bool expand_repeated_values;
    bool bump_displays;
    bool hide_inactive_displays;
    bool show_base_display_titles;
    bool show_dependent_display_titles;
    bool cluster_displays;
    unsigned char display_placement;
    unsigned int status_history_size;
    unsigned int popdown_history_size;
    bool sort_popdown_history;
    bool verify_buttons;
    bool auto_raise_tool;
    OnOff     decorate_tool;
    bool sticky_tool;
    Position  tool_right_offset;
    Position  tool_top_offset;
    bool button_captions;
    bool button_images;
    const char *button_caption_geometry;
    const char *button_image_geometry;
    bool flat_toolbar_buttons;
    bool flat_dialog_buttons;
    const char *button_color_key;
    const char *active_button_color_key;
    const char *splash_screen_color_key;
    bool auto_commands;
    const char *auto_command_prefix;
    int max_breakpoint_number;
    int max_display_number;
    bool splash_screen;
    bool color_wm_icons;
    ...
    int max_undo_size;
    const char *themes;
    const char *suppress_theme;
    bool maintenance;
    bool dump_core;
    bool debug_core_dumps;
    int lesstif_version;
    const char *dddinit_version;
